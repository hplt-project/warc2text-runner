...
def set_file_overlap(set1, file2, url_preprocess=lambda url: url.strip):
    """
    ...
    with gzip.open(file2, 'rt', encoding='utf-8') if file2.endswith('gz') else open(file2, 'rt', encoding='utf-8') as inp:
        overlap = set1.intersection((url_preprocess(s) for s in inp))
    return overlap
...
def overlap(file1, file2, mem_limit = 5*2**30, domain_level=False, debug=False):  # 1 GB memory limit
    """
    ...
    with gzip.open(file1, 'rt', encoding='utf-8') if file1.endswith('gz') else open(file1, 'rt', encoding='utf-8') as inp:
        while True:
            ...
            if not set1: break
            ss = set_file_overlap(set1, file2, url_preprocess=url_preprocess)
            overlap.update(ss)
