...
#    OUTDIR=$(dirname $x)
    ...
    echo "$(date) Processing $x, writing to ${OUTDIR}" 1>&2  # forward everything to stderr to interleave correctly with error messages from stage2local.sh
    stage2local.sh $x ${OUTDIR} || { rc="$?"; echo "ERROR: stage2local.sh exited with code $rc while processing $x" 1>&2; }  # in bash && and || have the same priority
    echo "$(date) stage2local_batch.sh planned exit code:" $rc 1>&2
done
