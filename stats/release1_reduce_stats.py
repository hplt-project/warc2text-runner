...
def load_stats(stats_dir,  key_position=-2):
    """
    Load data generated by text_stats.sh
    """
    stats_dir = Path(stats_dir)
    ddf = pd.read_csv(stats_dir/'doc_stats.csv', sep=r'\s+') 
    tdf = pd.read_csv(stats_dir/'text_stats.csv', sep=r'\s+')

    for df in [ddf, tdf]:
        p = df.path.str.split('/')
        df['lang'] = p.str[key_position]
        df['path'] = p.str.join('/')
...
def reduce_stats(stats_dir, key_position=-2):
    """
    ...
    """
    df = load_stats(stats_dir, key_position)
