...
find -L $GZ_DIR -name '*.jsonl.zst'|parallel --joblog $STATS_DIR/text_stats.log --eta --jobs=$NJOBS "set -o pipefail; echo {}|tr '\n' ' '; zstdcat {}|jq -r .text|wc --lines --words --chars --bytes" >>$STATS_DIR/text_stats.csv
echo "path docs" >$STATS_DIR/doc_stats.csv
find -L $GZ_DIR -name '*.jsonl.zst'|parallel --joblog $STATS_DIR/doc_stats.log --eta --jobs=$NJOBS "set -o pipefail; echo {}|tr '\n' ' '; zstdcat {}|wc --lines" >>$STATS_DIR/doc_stats.csv
