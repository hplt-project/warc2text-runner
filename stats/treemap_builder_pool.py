...
# For the format of pool statistics json files
...
if __name__ == "__main__":
    ...
    if stats_file.endswith(".tsv"):
        ...
        stats_df["lang"] = stats_df.index
        stats_df["chars"] = stats_df["lang_chars"]
        stats_df["docs"] = stats_df["lang_docs"]

    stats_df = stats_df[["lang", "chars", "docs"]]
    ...
    if args.exclude:
        ...
        lang_fam_df = lang_fam_df.loc[
            ~lang_fam_df["v3 Language Code (ISO 693-3+script)"].isin(args.exclude)
        ]

    stats_df = stats_df.sort_values("lang")
    # print(stats_df["lang"].values)
    # print(lang_fam_df["v3 Language Code (ISO 693-3+script)"].values)
